...
int main(int argc,char **argv) {
    ...
    if(argc<2) {
        fputs("Usage: ./gpm_extrema [opts] <input_field>\n\n"
              "Calculates the extremal values of a Gnuplot matrix field\n\n"
              "Options:\n"
    ...
    if(clean!=NULL) {
        ...
        // If the cleaning option is requested and the phi field dimensions
        // exactly match the main field dimensions, then just count each point
        // based on the corresponding phi value
        ...
        // If the cleaning option is requested and the phi field dimensions are
        // one smaller than the main field dimensions, then assume the two fields
        // are staggered with respect to each other. Count points depending on
        // a linear interpolation of the phi field.
        ...
        // There is a possibility that cleaning might result in no valid grid
        // points. In this case, output an error.
        if(start) {
